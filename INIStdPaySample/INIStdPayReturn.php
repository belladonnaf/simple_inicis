...
//		System.out.println("paramMap : "+ paramMap.toString());
            ...
            if (strcmp("0000", $_REQUEST["resultCode"]) == 0) {
                ...
                $signKey = "K0tZTnh2aGtXeVVaVTdPWUR1c3Z2dz09"; // 가맹점에 제공된 키(이니라이트키) (가맹점 수정후 고정) !!!절대!! 전문 데이터로 설정금지
                ...
                try {
                    ...
                    if (strcmp("0000", $resultMap["resultCode"]) == 0) {
                         ...
                         * **************************************************************************** */
                        $checkMap["mid"] = $mid;        // 필수					
                        $checkMap["authToken"] = $resultMap["authToken"];    // 필수					
                        $checkMap["applDate"] = $resultMap["applDate"];  // 필수					
                        $checkMap["applTime"] = $resultMap["applTime"];  // 필수					
                        $checkMap["timestamp"] = $resultMap["timestamp"];   // 필수					
                        $checkMap["signature"] = $resultMap["signature"];  // 필수				
...
// success
	$merchant_data = base64_decode($_POST['merchantData']);

	list($ord,$opt,$price,$buyername,$buyertel,$orderNumber) = explode('***',$merchant_data);
	
	$sql = " select ID from wp_posts where post_title = '".$ord."' limit 0,1 order by ID asc ";
	$product_id = $wpdb->get_var($sql);

	$title = "항목 $ord $opt / $buyername ($buyeremail / $buyertel / $orderNumber)";

	$sql = " INSERT INTO wp_posts (post_author, post_date, post_date_gmt, post_content, post_title, post_excerpt, post_status, comment_status, ping_status, post_password, post_name, to_ping, pinged, post_modified, post_modified_gmt, post_content_filtered, post_parent, guid, menu_order, post_type, post_mime_type, comment_count)  VALUES 
	(1, NOW(), NOW(), '".mysql_escape_string("항목 $ord <br>테마 $opt <br>가격 $price<br>구매자명 $buyername<br>구매자 Email $buyeremail<br>구매자 전화번호 $buyertel<br>주문번호 $orderNumber")."', '".mysql_escape_string($title)."', '', 'wc-completed', 'closed', 'closed', '', 'order', '', '', NOW() , NOW() , '', 0, '', 0, 'shop_order', '', 0); ";

	$wpdb->query($sql);
	$post_id = $wpdb->insert_id;	

	$sql = " update wp_posts set guid = '".get_site_url()."/?post_type=shop_order&#038;p=".$post_id."' where ID = ".$post_id;

	$sql = " INSERT INTO wp_woocommerce_order_items (order_item_name, order_item_type, order_id) VALUES (	'$ord', 'line_item', $post_id ); ";
	$wpdb->query($sql);
	
	$order_id = $wpdb->insert_id;

	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_qty', '1');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_tax_class', '');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_product_id', '".$product_id."');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_variation_id', '');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_line_subtotal', '".$price."');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_line_subtotal_tax', '');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_line_total', '".$price."');";
	$wpdb->query($sql);
	$sql = " INSERT INTO wp_woocommerce_order_itemmeta (order_item_id, meta_key, meta_value) VALUES ($order_id, '_line_tax', '');";
	$wpdb->query($sql);
	echo "<br><b>결제가 완료되었습니다. 2초 후에 창이 자동으로 닫힙니다</b>";
	echo "<script>window.opener.document.location.href='/';self.opener = self;setTimeout('window.close()',2000);</script>";
                    } else {
// fail
	echo "<br><b>결제에 실패하였습니다. 2초 후에 창이 자동으로 닫힙니다</b>";
	echo "<script>window.opener.document.location.href='/';self.opener = self;setTimeout('window.close()',2000);</script>";
...
?>
				...
				<input type='hidden' name='tid' value='<?php echo $resultMap["tid"]?>'/>
				</form>				
<?php
...
var _ref = getCookie('_referer');
window.location.href=_ref + '?ret_mode=pay_return&ret_msg=<?=base64_encode($_REQUEST['resultMsg'])?>';
</script>
<?
